...
export default {
  ...
  methods: {
    getCommentData() {
      get("/img2d/comments/admin/getAllForArticle").then(({data}) => {
        if (data.flag) {
    ...
    deleteComments(choose){
      post("/img2d/comments/admin/auditComments/Article/"+choose,this.checkItems).then(({data})=>{
        if(data.flag){
          successTips("操作成功")
          this.getCommentData()
        }
